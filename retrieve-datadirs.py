...
def main(args):
    ...
    if check_context_connectivity(kube_config, source_context):
        ...
        print(
            f'Source cluster connectivity failed. Please check the context definition in {kube_config}'
        )
        sys.exit(1)
    ...
    if target_context:
        if check_context_connectivity(kube_config, target_context):
            ...
            print(
                f'Target cluster connectivity failed. Please check the context definition in {kube_config}'
            )
            sys.exit(1)
    ...
    if mapping_file:
        if target_context:
            ...
            if is_valid_mapping_file(
                ...
                print(
                    'Mapping file does not match required structure or contains incorrect PVCs'
                )
        else:

...
class TestListPVs(unittest.TestCase):
    ...
    def test_list_pvs(
        self, mock_core_v1_api, mock_load_kube_config, mock_new_client_from_config
    ):
        ...
        mock_core_v1_api.assert_called_once_with(
            api_client=mock_new_client_from_config.return_value
        )
...
class TestGetBoundPvcs(unittest.TestCase):
    ...
    @patch('kubernetes.config.load_kube_config')
    @patch('kubernetes.config.new_client_from_config')
    def test_get_bound_pvcs(
        self, mock_new_client, mock_load_kube_config, mock_core_v1_api
    ):
...
class TestRetrievePVCsFromCluster(unittest.TestCase):
    ...
    def test_retrieve_pvcs_from_source(
        self, mock_print, mock_load_kube_config, mock_get_bound_pvcs, mock_write_file
    ):
        mock_get_bound_pvcs.return_value = ['default:test-pvc']

        retrieve_pvcs_from_clusters(
            'kube_config', 'source', 'source_context', 'target_context'
        )
        ...
        mock_print.assert_called_once_with(
            'PVCs for source-context source_context written to source_pvcs.txt'
        )
    ...
    @patch('kubernetes.config.load_kube_config')
    @patch('builtins.print')
    def test_retrieve_pvcs_from_target(
        self, mock_print, mock_load_kube_config, mock_get_bound_pvcs, mock_write_file
    ):
        mock_get_bound_pvcs.return_value = ['default:test-pvc']

        retrieve_pvcs_from_clusters(
            'kube_config', 'target', 'source_context', 'target_context'
        )
        ...
        mock_print.assert_called_once_with(
            'PVCs for target-context target_context written to target_pvcs.txt'
        )
    ...
    @patch('kubernetes.config.load_kube_config')
    @patch('builtins.print')
    def test_retrieve_pvcs_from_both(
        self, mock_print, mock_load_kube_config, mock_get_bound_pvcs, mock_write_file
    ):
        mock_get_bound_pvcs.side_effect = [
            ['default:source-pvc'],
            ['default:target-pvc'],
        ]

        retrieve_pvcs_from_clusters(
            'kube_config', 'both', 'source_context', 'target_context'
        )
...
class TestRetrieveSourceContext(unittest.TestCase):
    ...
    def test_retrieve_source_context(
        self, mock_list_kube_config_contexts, mock_load_kube_config
    ):
...
class TestRetrievePVs(unittest.TestCase):
    ...
    def test_retrieve_pvs_succesful_match(
        self, mock_match_pvs, mock_extract_values_from_pvs, mock_list_pvs
    ):
        mock_list_pvs.side_effect = [MagicMock(), MagicMock()]
    ...
    def test_retrieve_pvs_no_source_pvs(self, mock_list_pvs):
        mock_list_pvs.side_effect = [[], MagicMock()]

        with self.assertRaises(RuntimeError) as context:
            retrieve_pvs('kube_config', 'source-context', 'target-context')

        self.assertIn(
            'No PersistentVolumes were found in source context', str(context.exception)
        )
    ...
    def test_retrieve_no_target_pvs(self, mock_list_pvs):
        ...
        self.assertIn(
            'No PersistentVolumes were found in target context', str(context.exception)
        )
...
class TestMatchPVCs(unittest.TestCase):
    ...
    def test_match_pvcs_succesful(self, mock_print):
        ...
        mock_print.assert_any_call(
            "Source PV called pv1 matches with target PV called target-pv1"
        )
        mock_print.assert_any_call("Data dirs: /nfs/path1 /nfs/path1")
        mock_print.assert_any_call(
            "Source PV called pv2 matches with target PV called target-pv2"
        )
    ...
    def test_match_pvcs_no_match(self):
        ...
        self.assertIn(
            'PVCs in source and target cluster are not identical',
            str(context.exception),
        )


class TestSelectPVOnPVC(unittest.TestCase):
    def test_select_pv_on_pvs_succesful(self):
        pv1 = MagicMock(spec=V1PersistentVolume)
        pv1.spec.claim_ref.name = 'pvc1'
        pv1.spec.claim_ref.namespace = 'default'

        pv2 = MagicMock(spec=V1PersistentVolume)
        pv1.spec.claim_ref.name = 'pvc2'
        pv1.spec.claim_ref.namespace = 'default'

        pv_list = [pv1, pv2]

        result = select_pv_on_pvc(pv_list, 'default', 'pvc1')


class TestRetrieveDirsFromMappingFile(unittest.TestCase):
    @patch('functions.get_bound_pvcs')
    @patch('functions.select_pv_on_pvc')
    @patch('kubernetes.client.CoreV1Api')
    @patch('kubernetes.config.new_client_from_config')
    @patch('kubernetes.config.load_kube_config')
    @patch(
        ...
        read_data='default:pvc1,default:pvc2\nkube-system:pvc3,kube-system:pvc4',
    )
    def test_retrieve_dirs_from_mapping_file_succesful(
        self,
        mock_file,
        mock_load_kube_config,
        mock_new_client_from_config,
        mock_core_v1_api,
        mock_select_pv_on_pvc,
        mock_get_bound_pvcs,
    ):
        mock_get_bound_pvcs.side_effect = [
            ['default:pvc1', 'kube-system:pvc3'],
            ['default:pvc2', 'kube-system:pvc4'],
        ]

        source_pv_mock = MagicMock()
        source_pv_mock.items = [MagicMock(status=MagicMock(phase='Bound'))]
        target_pv_mock = MagicMock()
        target_pv_mock.items = [MagicMock(status=MagicMock(phase='Bound'))]
        mock_core_v1_api.return_value.list_persistent_volume.side_effect = [
            source_pv_mock,
            target_pv_mock,
        ]

        mock_select_pv_on_pvc.side_effect = [
            MagicMock(),
            MagicMock(),
            MagicMock(),
            MagicMock(),
        ...
        retrieve_dirs_from_mapping_file(
            "fake_mapping_file", "fake_kubeconfig", "source-context", "target-context"
        )

        self.assertEqual(
            mock_file.call_count, 3
        )  # Since open is called 3 times in the function
        self.assertEqual(mock_get_bound_pvcs.call_count, 2)
        self.assertEqual(mock_select_pv_on_pvc.call_count, 4)
    ...
    def test_retrieve_dirs_from_mapping_file_invalid_format(
        self, mock_file, mock_get_bound_pvcs
    ):
        with self.assertRaises(RuntimeError) as context:
            retrieve_dirs_from_mapping_file(
                "fake_mapping_file",
        ...
        self.assertIn("Error found in line 2", str(context.exception))
    ...
    def test_retrieve_dirs_from_mapping_file_missing_pvcs(
        self, mock_file, mock_get_bound_pvcs
    ):
        mock_get_bound_pvcs.side_effect = [
            ['default:pvc1'],
        ...
        self.assertIn(
            'Source PVC kube-system:pvc3 from mapping file was not found in the source cluster',
            str(context.exception),
        )
